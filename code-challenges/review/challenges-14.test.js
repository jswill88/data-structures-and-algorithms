...
let biggerThanLuke = (arr) => {
  let luke = arr.filter(obj => obj.name === 'Luke Skywalker')[0];
  return arr.reduce((massArray, obj) => {
    if (parseInt(obj.mass) > parseInt(luke.mass)) {
      massArray = massArray.length === 0 ? obj.name :
        `${massArray} - ${obj.name}`
    }
    return massArray;
  }, '')

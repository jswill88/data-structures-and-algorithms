...
const count = (target, input) => input
  .reduce((currentTotal, array) => {
    array.forEach(value => {
      if (value === target) {
        currentTotal++;
      }
...
const totalSum = (input) => input
  .reduce((currentSum, array) => currentSum += array
...
const divisibleByFiveTwoToThePower = (input) => input
  .map(array => array
    .reduce((subArray, value) => {
      if (!(value % 5) && typeof value === 'number') {
        subArray.push(Math.pow(2, value));
      }
      return subArray;
    }, []));
...
let findShortest = (data) => data
  .reduce((returnObj, obj) => {
    if (parseInt(obj.height) < parseInt(returnObj.height)) {
      returnObj.height = obj.height;

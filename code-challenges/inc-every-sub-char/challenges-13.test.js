...
const firstLetters = (arr) => arr.map(string => string.slice(0, 1));
...
const findHappiness = (arr) => arr
  .reduce((returnArray, currentString) => {
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(phoneNum => {
    return phoneNum.substring(1, 4) + phoneNum.substring(6, 9) + phoneNum.substring(10, 15);
  });
}
...
const allHappy = (arr) => arr
  .reduce((bool, string) => !string.includes(':)') ?
    false : bool, true);
...
const findAnything = (arr, target) => arr
  .filter(string => string.includes(target));
...
const findEvery = (arr, target) => arr
  .reduce((bool, string) => !string.includes(target) ?
    false : bool, true);
...
const sortByDay = (arr) => {
  let newArray = [[],[],[],[],[],[],[]];
  arr.forEach(string => daysOfWeek.forEach((day,i) =>{
    if (string.includes(day)){
      newArray[i].push(string);
    }
  }))
  return newArray;
};
...
const characterByIndex = (arr) => arr.map((v,i) => v[i]);

...
const templatingWithMustache = () => {
  ...
  characters.forEach(character => {
    ...
    let htmlTemplate = Mustache.render(template, character);
...
const houseSize = (arr) => {
  return arr.map(house => {
    let size = 1 + (house.spouse ? 1 : 0) + house.children.length;
    return {house: house.house, members: size};
  })
};
...
const houseSurvivors = (arr) => {
  return houseSize(arr).map((obj,i) => {
